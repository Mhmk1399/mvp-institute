...
export const promptIdentity: PromptIdentity = { id: "class-realtime-reply", version: "v1" };

export interface ClassRealtimeReplyInput {
  level: string;
  subject: string;
...
export function buildRealtimeReplyInstructions(input: ClassRealtimeReplyInput): string {
  ...
  return [
    ...
    "Follow the approved teaching move exactly. Never reveal the plan or any internal reasoning.",
    ...
    plan.responsePlan.correctionApproach === "none"
      ...
      : `Weave the corrected expression in naturally (${plan.responsePlan.correctionApproach}).`,
    ...
    "Deliver naturally and warmly as speech. No markdown, no lists unless truly necessary,",
    "no sound effects, and no singing.",
    "",
